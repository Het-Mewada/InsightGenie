...
const DashboardCharts: React.FC<Props> = ({ data, xAxisKey, excludeKeys = [] }) => {
  // Handle empty data
  ...
  if (numericKeys.length === 0) {
    return <p className="text-yellow-400">No numeric data available for charts</p>;
  }
  ...
  const labels = data.map((item) => {
    ...
    return typeof value === "number" ? ` ${xAxisKey} ${value}` : String(value);
  });
